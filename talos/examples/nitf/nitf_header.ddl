...
-- FIXME
def FSCODE = Many 11 (Match1 ' ') -- Chunk 11 DigraphSeq 
...
def FS_declass = { 
  declass_type = FSDCTP; 
  fsdcdt = case declass_type is {
    dd -> CallMeMaybe Date2;
    _  -> { Spaces 8; ^ nothing }
  };
  fsdcxm = case declass_type is {
    x -> CallMeMaybe FSDCXM;
    _ -> { Spaces 4; ^ nothing }; 
  }; 
  fsdg = case declass_type is {
    gd, ge -> CallMeMaybe FSDG;
    _       -> { Spaces 1; ^ nothing }; 
  }; 
  fsdgdt = case declass_type is {
    gd -> CallMeMaybe Date2;
    _  -> { Spaces 8; ^ nothing}; 
  };
  fscltx = case declass_type is {
    de, ge -> CallMeMaybe (@Many 43 ECSA);
    _       -> { Spaces 43; ^ nothing } 
  }
} 
