...
basic_json<> exportJSON(User::JSON_value x) {
  ...
  // 
  // using DDLValue = std::option<
  //                     User::JSON_value,
  //                     DDL::Array<JSON_value>
  //                  >;
  // std::vector<DDLValue> todo = {std::option(x)};
  // std::vector<std::pair<DDLValue,size_t>> in_progress;
  // std::vector<basic_json<>> done;
// 
  // while (!todo.empty()) {
  //   DDLValue el = todo.back();
  //   todo.pop_back();
  //   switch(el.index()) {
  //     case 0: {
  //       auto el = el.get<JSON_value>();
// 
  //       switch(el.getTag()) {
  //   
...
// 
  ...
  //           in_progress.push_back({std::option{el},n});
  //           continue;
  //         }
  //   
  //         case DDL::Tag::JSON_value::Object: {
  //           // auto map = el.borrow_Object();
  //   
  //           break; // continue
  //         }
  //       }
  ...
  //     }
  //     case 1: {
  //       auto el = el.get<DDL::Array<JSON_value>>();
  //       auto n = el.size().rep();
