...
def Field_name =
  block
    ...
    build (many (buf = emit builder (toLower $alpha))
                emit buf (toLower $[ $alpha | '-' | $digit ]))
...
def HTTP_field_content = 
    build (                                                                        
      many (buf = builder)                                                         
        block                                                                      
          let buf1 = many (space = buf) emit space $ows
          emit buf1 $http_field_vchar                                         
    )      
