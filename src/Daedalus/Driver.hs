...
import Daedalus.PP(pp,vcat,(<+>),nest,($$),colon,backticks,bullets,Doc)
...
passPP :: ModuleName -> Daedalus (String, Doc)
passPP m =
  do mb <- getLoaded m
     pure $ case mb of
       Nothing -> ("nothing", mempty)
       Just ModuleInProgress                -> ("module in progress", mempty)
       Just (ParsedModule md) -> ("parsed", ppDoc md)
       Just (ResolvedModule md) -> ("resolved", ppDoc md)
       Just (TypeCheckedModule md) -> ("type checked", pp md)
       Just (DeadValModule md)     -> ("removed dead values", pp md)
       Just (SpecializedModule md) -> ("specialised", pp md)
       Just (CoreModule md)        -> ("core", pp md)
       Just (VMModule md)          -> ("vm", pp md)
