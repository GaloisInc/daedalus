...
coreToCore :: String -> Bool -> (Core.Module -> PassM Core.Module) -> ModuleName ->  Daedalus ()
coreToCore name doNorm f m =
  do ph <- doGetLoaded m
     case ph of
       CoreModue ast ->
         do i <- ddlRunPass (f ast)
            let i' = if doNorm then Core.normM i else i
            ddlUpdate_ \s ->
              s { loadedModules =
                     Map.insert m
                        (CoreModue i')
...
passInline what no = coreToCore "inline" True (Core.inlineModule what no)

passStripFail :: ModuleName -> Daedalus ()
passStripFail = coreToCore "strip Fail" False (pure . Core.stripFailM)

passNoLoops :: ModuleName -> Daedalus ()
passNoLoops = coreToCore "no loops" False Core.noLoop

passNoBitdata :: ModuleName -> Daedalus ()
passNoBitdata = coreToCore "no bitdata" False Core.noBitdata

passSpecTys :: ModuleName -> Daedalus ()
passSpecTys = coreToCore "specialise types" False Core.specialiseTypes

passConstFold :: ModuleName -> Daedalus ()
passConstFold = coreToCore "specialise types" False Core.constFold

passDeterminize :: ModuleName -> Daedalus ()
passDeterminize = coreToCore "determinize" False (pure . Core.determinizeModule)

passNorm :: ModuleName -> Daedalus ()
passNorm = coreToCore "norm" True pure
