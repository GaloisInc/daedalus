module Daedalus.RTS.Vector
  ...
  , Daedalus.RTS.Vector.foldl'
  , Daedalus.RTS.Vector.ifoldl'
  , Daedalus.RTS.Vector.foldM'
  , Daedalus.RTS.Vector.ifoldM'
...
foldl' :: (VecElem a) => (s -> a -> s) -> s -> Vector a -> s
foldl' f s0 (Vec xs) = vFoldl' f' s0 xs
  where f' s a = f s (reading a)

ifoldl' :: (VecElem a) => (s -> Int -> a -> s) -> s -> Vector a -> s
ifoldl' f s0 (Vec xs) = vIFoldl' f' s0 xs
  where f' s i a = f s i (reading a)

foldM' :: (VecElem a, Monad m) => (s -> a -> m s) -> s -> Vector a -> m s
foldM' f s0 (Vec xs) = vFoldM' f' s0 xs
  where f' s a = f s (reading a)

ifoldM' ::
  (VecElem a, Monad m) => (s -> Int -> a -> m s) -> s -> Vector a -> m s
ifoldM' f s0 (Vec xs) = vIFoldM' f' s0 xs
  where f' s i a = f s i (reading a)
