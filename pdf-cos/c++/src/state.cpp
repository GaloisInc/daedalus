...
StreamThunk::StreamThunk(uint64_t refid, uint64_t container, uint64_t index)
: refid(refid), container(container), index(index) {}

bool
StreamThunk::getDecl(DDL::Input input, User::TopDecl *result)
{
    ...
    if (index >= stream->size().value) {
        return false;
    }

    User::TopDeclDef topDef;
    topDef.init_value(stream.borrow()[index]);
...
// Owns topDecl
void
ReferenceTable::register_topdecl(uint64_t refid, uint16_t gen, User::TopDecl topDecl)
{
    auto key = std::make_pair(refid, gen);
    table.insert_or_assign(key, owned(topDecl));
}
...
)
{
    ...
    table.insert(
        std::make_pair(std::make_pair(refid, 0), StreamThunk(refid, container, index))
    );
}
...
ReferenceTable::resolve_reference(
    ...
    if (auto *thunk = std::get_if<TopThunk>(&entry)) {
        cursor->second = Blackhole();
        User::TopDecl decl;
        bool success = thunk->getDecl(input, &decl);
        if (success) {
            cursor->second = borrowed(decl);
            *result = decl;
        }
        return success;
    }
        ...
        for (DDL::Size j = 0; j < entries.size(); j.increment()) {
            ...
            switch (entry->getTag()) {
                case DDL::Tag::XRefObjEntry::compressed: {
                    ...
                    std::cerr << "Adding compressed xref " << refid << " at " << obj_index << std::endl;
